...
当前 VSCode 扩展版本：`1.4.9`。

`1.4.x` 系列重写了语言服务核心，并补齐 `.dsf`、动态 MaterialExpression fallback、`VolumeTexture` / `Texture3D`、`Layout` / Graph region 和 Function 内置函数 metadata。补全、诊断、语义高亮、折叠和文档符号现在通过 scanner / parser / context / symbol / provider 模块处理，减少不同 section 和作用域之间的误判。
