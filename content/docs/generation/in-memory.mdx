...
**存储决定一次重建怎么落地，而不是发起编译的那一方决定。** <Since v="1.8.0" /> 一次纯内存编译落在磁盘上
已经存在的 package 上时，资产会被重建**并保存**，而不是在自己的文件之上于内存里重建。生成成功，并记一条：
...
<Callout type="warn">
旧行为产出的是一个**既不等于磁盘上的文件、也不等于将来会被写出的任何东西、还自称干净**的对象 ——
于是一次 Save All 就能落下一个谁也没选过的状态，而你在编辑器里看到的版本会在重启后消失。
现在「这个资产到底是什么」重新只有一个答案：上一次编译产出的东西，无论这个资产住在哪。
</Callout>

一个值得知道的推论：**启动扫描不再强制重建**。在每个内存资产无论如何都会重建时，强制是免费的；
一旦磁盘资产开始被*保存*，它就不免费了 —— 每次启动都会重写每一个持久化的生成资产，带来磁盘写入、
版本控制噪音和更慢的启动，而这些重建早已被 source hash 排除掉了。现在启动遵守
[source hash 跳过](/docs/generation/regeneration)，所以一个已经最新的已保存资产不会被动。
改 **Default Compiler Backend** 仍然强制，因为哈希看不见那个设置。
...
（自 `1.8.0` 起，这些资产其实不再*遮蔽*任何东西 —— 它们只是仍然在磁盘上，并且就在磁盘上被重建。
...
| 资产注册表 | 生成实际写出的每个 package 都会在 commandlet 的 `Main` 之前交给 `IAssetRegistry::ScanModifiedAssetFiles` <Since v="1.8.0" /> |
| 失败时 | **中止整个 cook** |
...
<Callout type="info">
**注册进资产注册表这一步是必要的，不是保险。** <Since v="1.8.0" /> 一个 cook 请求是经
`IAssetRegistry::DoesPackageExistOnDisk` 解析的，而这个函数只查注册表的内存状态、没有文件系统兜底 ——
而生成恰好跑在注册表枚举完内容目录**之后**。所以在此之前，刚写出的 package 对那次查询是不可见的：
...
- 自 `1.6.0` 起，**纯内存材质也会跑自动布局**，所以保存之后你看到的图和生成资产是一样的。
  *Project Settings ▸ … ▸ Compiler ▸ Lay Out In-Memory Graphs* 可以关掉它，关掉之后这些图保留构建阶段的
  坐标 —— 一根高柱子。见[项目设置](/docs/settings/project)。
