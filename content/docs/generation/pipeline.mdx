...
| 6 | 计算构建键 | 对 **预处理后** 文本做 CRC32，并折进默认 backend、三张映射表、插件版本和引擎版本 <Since v="1.8.0" /> | — |
...
7. 连接每一条 `Outputs` 绑定。
8. 布局。纯内存模式下**也会运行** <Since v="1.6.0" />，可用 *Lay Out In-Memory Graphs* 关掉；表达式数量达到 1200 及以上且没有 [`Layout`](/docs/language/layout) section 时跳过。
9. 重新编译材质。
...
| 编辑器启动扫描 | **否** <Since v="1.8.0" /> | 内存 / 磁盘 | 遵守构建键，所以已经最新的资产不会被动 |
| *Generate all in-memory materials* —— 每次修改 **Default Compiler Backend** | 是 | 内存 | 重编所有项目源文件 |
...
<Callout type="info">
**一批文件按依赖顺序编译** <Since v="1.8.0" />。一个调用 `ShaderFunction` 的 `.dsm` 会把调用节点绑到
**活着的** `UMaterialFunction` 资产上 —— `SetMaterialFunction` 是从对象而不是从源码上读 pin 的 ——
所以先编调用方就绑到了那个函数上一版的接口。两个入口（watcher 的待编译批次、全工程扫描）都按 import 图
做拓扑排序；只有批次内部的边被考虑，成环留给 import 加载器用自己的诊断拒绝。
</Callout>
...
一个例外自 <Since v="1.8.0" /> 起：**存储决定一次重建怎么落地**。落在一个磁盘上已经存在的 package 上的
纯内存编译，会在磁盘上重建并保存，而不是在自己的文件之上于内存里重建。
</Callout>
