...
**Storage decides how a rebuild persists, not the compile that asked for it.** <Since v="1.8.0" /> A
memory-only compile that lands on a package which already exists on disk rebuilds the asset *and
saves it*, rather than rebuilding it in memory over the top of its own file. Generation succeeds and
logs:
...
chose, and the version you saw in the editor vanished on restart. There is now exactly one answer to
"what is this asset": whatever the last compile produced, wherever the asset lives.
</Callout>
...
disk churn, source-control noise and a slow startup, for rebuilds the source hash had already ruled
out. Startup now respects the [source-hash skip](/en/docs/generation/regeneration), so an up-to-date
saved asset is left alone. Changing the **Default Compiler Backend** still forces, because the hash
cannot see that setting.

Two commands address the persisted assets themselves:
...
(Since `1.8.0` those assets are not *shadowing* anything any more — they are simply still on disk, and
are rebuilt there. The notification points at the cleaner for anyone who wants the memory-only end
state.)
...
| Registry | every package the generation actually wrote is handed to `IAssetRegistry::ScanModifiedAssetFiles` before the commandlet's `Main` <Since v="1.8.0" /> |
...
<Callout type="info">
**Registering with the AssetRegistry is load-bearing, not belt-and-braces.** <Since v="1.8.0" /> A
cook request is resolved through `IAssetRegistry::DoesPackageExistOnDisk`, which consults only the
registry's in-memory state and has no filesystem fallback — and generation runs *after* the registry
enumerated the content directories. So a freshly written package used to be invisible to that lookup:
`DirectoriesToAlwaysCook` scanned the file off disk and then the request was dropped again, with no
error anywhere — the material simply was not in the pak, and `LoadObject` failed at runtime in a
Shipping build. The run now logs
...
- Since `1.6.0`, **automatic layout runs on memory-only materials too**, so the graph you see after a
  save matches the generated asset. *Project Settings ▸ … ▸ Compiler ▸ Lay Out In-Memory Graphs* turns
  it back off, and those graphs then keep their construction coordinates — a single tall column. See
