...
| 6 | Build key | CRC32 of the **prepared** text, folded together with the default backend, the three mapping tables, the plugin version and the engine version <Since v="1.8.0" /> |  — |
...
8. Lay the graph out. **Runs in memory-only mode too** <Since v="1.6.0" />, unless *Lay Out
   In-Memory Graphs* is off; still skipped for graphs of 1200 or more expressions unless a
   [`Layout`](/en/docs/language/layout) section is present.
...
| The editor's startup sweep | **no** <Since v="1.8.0" /> | memory / disk | respects the build key, so an up-to-date asset is left alone |
| *Generate all in-memory materials* — every change to **Default Compiler Backend** | yes | memory | every project source recompiled |
...
<Callout type="info">
**A batch of source files compiles in dependency order** <Since v="1.8.0" />. A `.dsm` that calls a
...
first bound it against the previous version of that function's interface. Both drain points, the
watcher's pending-file batch and the whole-project sweep, topologically sort by the import graph; only
edges inside the batch are honoured, and a cycle is left for the import loader to reject with its own
diagnostic.
</Callout>
...
One exception since <Since v="1.8.0" />: **storage decides how a rebuild persists**. A memory-only
compile that lands on a package which already exists on disk rebuilds and saves it there, rather than
rebuilding it in memory over the top of its own file.
</Callout>
