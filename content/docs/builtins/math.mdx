---
title: 数学内置
description: Graph 块直接降级为算术材质节点的 19 个无前缀 HLSL 风格调用名。
---

有 19 个无前缀、按 HLSL 拼写的调用名会被直接降级为算术 `UMaterialExpression` 节点。它们覆盖 16 种
运算 —— 其中三个名字是别名。
...
<Callout type="warn">
**这 19 个名字是保留名，会静默遮蔽你自己的代码。** 命名为 `lerp`、`clamp`、`dot`、`min`、`max`、
...
这个调用面上没有矩阵、反三角、指数、对数、`step`、`smoothstep`、`reflect`、`refract`、`cross`
或 `length` 内置节点。有两条出路：

- 直接触达对应的 `UMaterialExpression`，例如
  `UE.Expression(Class = "CrossProduct", OutputType = "float3", A = u, B = v)`；
- 把运算写在 [`Function`](/docs/language/functions) 的 HLSL 体里，那里可以用完整的 HLSL 内建函数集。
...
<Diag>
  ...
  <D msg="Math function '{Name}' expects exactly 2 arguments." cause="同上，适用于 dot、pow、min、max、fmod 和 mod。" />
  <D msg="Math function '{Name}' expects exactly 3 arguments." cause="同上，适用于 lerp、mix 和 clamp。" />
