...
description: How import assembles several files into one translation unit — recognition, specifier normalisation, source roots and root qualifiers, cycles, and line mapping.
---
...
import "Plugin.MoonToon:Shared/Toon.dsh";                 // -> another root, see below <Since v="1.6.0" />
```

A bare `<path>` resolves inside the importing file's **own source root**. The `<root-qualifier>`
prefix <Since v="1.6.0" /> is the one thing that lets an import reach a different root.
...
| Plugin source <Since v="1.6.0" /> | `<Plugin>/DShader` | *Scan Plugin Source Directories* |
...
The vocabulary is the one [`Root=`](/en/docs/generation/asset-paths) already uses, minus the
...
same way. There is **no** relative-to-the-importing-file candidate, and the importing file's own root
is not consulted.
...
| a qualifier naming no live root <Since v="1.6.0" /> | `DreamShader import '{Specifier}' referenced from '{Path}' names source root '{Qualifier}', which is not a DreamShader source root.` — a misspelt plugin name, a disabled plugin, or one that ships no `DShader` folder |
...
<Diag>
  ...
  <D msg="DreamShader import '{Specifier}' referenced from '{Path}' names source root '{Qualifier}', which is not a DreamShader source root." cause="A root-qualified import whose qualifier matched the grammar but named no live root — a misspelt plugin name, a disabled plugin, or one shipping no DShader folder." fix="Check the plugin name, and that the plugin is enabled and ships a DShader folder." />
