...
DreamShader 是一个 Unreal Engine 材质生成插件。它提供 DreamShaderLang 源语言，让材质、材质函数、Material Layer 和共享 helper 可以像代码一样维护，并生成标准 Unreal `UMaterial`、`UMaterialFunction`、`UMaterialFunctionMaterialLayer` 与 `UMaterialFunctionMaterialLayerBlend` 资产。

当前插件版本：`1.3.9`。插件目前基于 Unreal Engine `5.7` 开发，其他版本尚未完整测试。
...
| DreamShader 插件 | 解析源文件、生成 Unreal 节点、写入资产、保存 metadata，并导出 Bridge 诊断和 MaterialExpression manifest。 |
| VSCode 扩展 | 提供编辑器语言服务、动态 MaterialExpression 补全、Bridge 诊断和 Package 工作流入口。 |
