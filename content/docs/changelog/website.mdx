...
- 更新 VSCode 扩展 ChangeLog，记录 `1.5.1` 到 `1.5.3` 的 Template、Bridge、Package、callable Hover 和材质预览能力。
...
- 新增 [布局与 Region](../syntax/layout.mdx) 页面，记录 `Layout = { Node(...); Comment(...); }` 与 Graph `#Region` / `#EndRegion`。
- 补充反编译导出 `.dsm` / `.dsf`、`VolumeTexture` / `Texture3D`、Function 内置函数 metadata、MaterialExpression fallback manifest 的说明。
- 补充 README 中的 Rider 语言支持仓库和能力说明，编辑器支持页现在同时覆盖 VSCode 与 Rider。
- 首页增加轻量入场与卡片悬浮动画，并尊重 `prefers-reduced-motion`。
- 更新 DreamShader 插件与 VSCode 扩展 ChangeLog，记录插件 `1.3.3` 到 `1.3.9`、VSCode `1.4.1` 到 `1.4.9` 的主要变化。
...
- 更新 DreamShader 插件与 VSCode 扩展 ChangeLog，记录 `1.2.10`、`1.3.0`、`1.3.1`、`1.3.2`、VSCode `1.2.27`、`1.3.x` 和 `1.4.0` 的主要变化。
...
- 更新输出绑定参考，补充 `Base.CustomizedUVs0..7` 别名、旧版 Color 输出、`SurfaceThickness`、`Displacement` 和 Moon Engine 专属 `MooaEncodedAttribute0..4`。
