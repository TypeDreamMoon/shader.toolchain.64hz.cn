...
`bridge.db`。两者都只被 `DreamShaderEditor` 使用。

共四个模块：`DreamShaderLang` 和 `DreamShader`（Runtime，`PostConfigInit` 阶段加载），以及
`DreamShaderCompiler` 和 `DreamShaderEditor`（Editor，`Default` 阶段）。只有两个 Editor 模块会生成资产。
...
完整面板有十五项属性，下面这几项是第一天就会碰到的。注意配置标识符保留了 `b` 前缀，而显示名会把它去掉。
...
`[/Script/DreamShader.DreamShaderSettings]`，因此会被所有拉取该项目的人共享。十五项设置全部记录在
...
DreamShader `1.8.0` 基于 Unreal Engine `5.8` 开发，并在 Win64 上用单插件
