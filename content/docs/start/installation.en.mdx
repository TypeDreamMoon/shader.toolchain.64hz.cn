...
second backs the editor bridge's `bridge.db`. Both are used only by `DreamShaderEditor`.

Four modules load: `DreamShaderLang` and `DreamShader` (Runtime, at `PostConfigInit`), then
`DreamShaderCompiler` and `DreamShaderEditor` (Editor, at `Default`). Only the two Editor modules build
assets.
